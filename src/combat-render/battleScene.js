...
function basePose(combatant,x,y,side){return{name:combatant.name,x,y,side,lean:0,flash:0,weapon:null,defeated:combatant.hp<=0};}
...
export function playCombatAnimation({canvas,beforeState,afterState,events,martialId}){
  ...
  return new Promise(resolve=>{
    function frame(now){
      ...
      left.weapon=after.left.id===attackerId?weaponFor(martialId):null;right.weapon=after.right.id===attackerId?weaponFor(martialId):null;
      const attacker=after.left.id===attackerId?left:right,target=after.left.id===targetId?left:right,dir=attacker.side;
      const travel=120;
      if(event){
        ...
        if(event.type==='recover'){attacker.x+=dir*travel*(1-p);target.lean*=1-p;}
        if(event.type==='defeat'){target.lean=dir*(Math.PI/2)*easeOut(p);target.y+=25*easeOut(p);}
      }
      const hitClip=timeline.clips.find(x=>x.event.type==='hit'),hitStarted=hitClip&&elapsed>=hitClip.startMs;
      const leftHp=left.id===undefined?after.left.hp:(after.left.id===targetId&&hitStarted?after.left.hp:before.left.hp),rightHp=after.right.id===targetId&&hitStarted?after.right.hp:before.right.hp;
