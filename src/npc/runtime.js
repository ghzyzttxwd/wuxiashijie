...
export function createNpcState(npc,hour=8){
  const slot=routineAt(npc,hour),hidden=Boolean(npc.visibility?.hidden);
  return{id:npc.id,alive:true,health:100,locationId:slot.locationId||npc.homeLocationId,sceneId:slot.sceneId,activity:slot.activity,currentGoal:npc.publicGoals[0]||null,flags:{...(npc.stateFlags||{}),hidden,discovered:!hidden}};
}
