...
import {ensureProgression,applyTrainingProgress}from'../player/progression.js';
export const SESSION_SCHEMA=1;
function normalizeCharacter(character){return ensureProgression(ensureMartialState(character));}
...
export function createSession(character){const normalized=normalizeCharacter(character),worldState={npc:createNpcWorld(normalized.world),events:createEventWorld(normalized.world),factions:{}};return{schema:SESSION_SCHEMA,character:normalized,worldState:advanceEventWorld(worldState,normalized.world)};}
export function ensureSession(session){if(!session?.character)throw Error('invalid_session');const character=normalizeCharacter(session.character),raw=session.worldState||{},worldState={...raw,factions:raw.factions||{},events:ensureEventWorld(raw.events,character.world),npc:raw.npc||createNpcWorld(character.world)};const normalized={...session,character,worldState};return syncSessionWorld(normalized);}
export function syncSessionWorld(session){if(!session?.character)throw Error('invalid_session');return{...session,character:normalizeCharacter(session.character),worldState:syncWorldState(session.worldState,session.character.world)};}
...
export function trainMartialSession(session,martialId,hours=24){const s=ensureSession(session),trained=trainMartial(s.character,martialId,hours),progressed=applyTrainingProgress(trained.character,martialId,hours),world=advanceWorldTime(progressed.world,hours),character={...progressed,world};return{session:{...s,character,worldState:syncWorldState(s.worldState,world)},result:trained.result};}
