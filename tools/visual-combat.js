...
async function captureImpact(){installSyntheticRaf();const result=await animate('martial_basic_sword','基础剑法 · 命中瞬间',{freezeAt:{eventType:'hit',progress:.6}});document.body.dataset.visualReady=result?.frozen?'impact':'failed';document.body.dataset.visualProgress=String(result?.progress??0);}
