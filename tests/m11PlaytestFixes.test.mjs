...
function make(talentId=null){let c=createCharacter({originId:'hunter',talentId,baseAttributes:BASE});c=learnMartial(c,'martial_basic_sword',{commonAccess:true});return c;}
function setSword(c,mastery,understanding){return{...c,martialState:{...c.martialState,learned:{...c.martialState.learned,martial_basic_sword:{...c.martialState.learned.martial_basic_sword,mastery,understanding}}}};}
function place(s,location,scene){return{...s,character:{...s.character,world:{...s.character.world,location,scene}}};}

test('天生剑骨不是悟性皮肤：剑法修炼倍率真实高于凡骨',()=>{const normal=trainMartial(make(), 'martial_basic_sword',24),bone=trainMartial(make('sword_bone'),'martial_basic_sword',24);assert.ok(bone.result.masteryGain>normal.result.masteryGain*1.5);assert.ok(bone.result.understandingGain>normal.result.understandingGain*1.5)});
...
test('基础吐纳修炼会推动真实内力阶段和江湖层级',()=>{let c=createCharacter({originId:'hunter',baseAttributes:BASE});c=learnMartial(c,'martial_basic_breathing',{commonAccess:true});let s=createSession(c);for(let i=0;i<15;i++)s=trainMartialSession(s,'martial_basic_breathing',24).session;assert.notEqual(innerStage(s.character).id,'none');assert.notEqual(jianghuRank(s.character).id,'unranked')});
test('熟练基础武功不能靠一场战斗从三流跳二流，二流至少要主修大成',()=>{let c=make();c=setSword(c,4500,15);c=addProgress(c,{experience:1200,innerExperience:450});assert.equal(jianghuRank(c).id,'third_rate');c=applyBattleProgress(c,{difficulty:3,win:true});assert.equal(jianghuRank(c).id,'third_rate');c=setSword(c,7000,20);assert.equal(jianghuRank(c).id,'second_rate')});
test('基础入门品级武功不能单独把人堆到一流以上',()=>{let c=setSword(make(),10000,100);c=addProgress(c,{experience:30000,innerExperience:7000});assert.equal(jianghuRank(c).id,'second_rate')});
